                        ...
                        <tbody>
                            ...
                            <?php $__currentLoopData = $details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    ...
                                    <td><?php echo Str::words($detail->description, 15, '....'); ?>

                                         <?php if(Str::of($detail->description)->wordCount() > 15): ?>
                                            <a  class="editdes" style="cursor: pointer;" data-description="<?php echo e($detail ->description); ?>">
                                                See More
                                            </a>
                                        <?php endif; ?>
                                    </td>
                                    <td><span
                                            class="badge badge-<?php echo e($detail->active_status == 0 ? 'danger': 'success'); ?>"><?php echo e($detail->active_status == 0 ? 'Inactive': 'Active'); ?></span>
                                    </td>
                                    <td><?php echo e($detail->created_at->toFormateDate()); ?></td>
                                    <td>    
                                        <button type="button" 
                                            ...
                                            <div class="dropdown-menu text-center bg-light-blue">
                                                <?php if($detail->active_status == 0): ?>
                                                   <a href="<?php echo e(route('update-home-status', ['id' => $detail->id , 'status' => $detail->active_status ])); ?>" class="btn btn-success btn-sm btn-block"><i class="fas fa-angle-double-right"></i> Active</a> 
                                                <?php else: ?>
                                                    <a href="<?php echo e(route('update-home-status', ['id' => $detail->id , 'status' => $detail->active_status ])); ?>" class="btn btn-danger btn-sm btn-block"><i class="fas fa-angle-double-right"></i> Inactive</a> 
                                                <?php endif; ?>
                                                <button type="button" data-id="<?php echo e($detail->id); ?>"
                                                    data-title="<?php echo e($detail->title); ?>"
                                                    data-description="<?php echo e($detail->description); ?>"
                                                    data-active_status="<?php echo e($detail->active_status); ?>"
                                                    class="btn btn-success btn-sm editData  btn-block">
                                                    <i class="fas fa-edit"></i> Edit
                                                </button>

                                                <a href="<?php echo e(url('delete-home-details/'. $detail->id)); ?>"
...
<script>
    $(document).ready(function () {
            ...
            $("#formData").submit(function (e) { 
              ...
              $.ajax({
                  type: "POST",
                  url: "<?php echo e(route('store-details')); ?>",
                  contentType: false,
                  processData: false,
                  headers: {
          ...
          $("#editData").submit(function (e) { 
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('update-details')); ?>",
                    contentType: false,
                    processData: false,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
<?php echo $__env->make('backend.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\mail_sale_another\resources\views/backend/homepage/homepaveshop/indexPaveshop.blade.php ENDPATH**/ ?>